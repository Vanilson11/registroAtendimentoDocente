...
public class ReportPdfByCoordinatorUseCase : IReportPdfByCoordinatorUseCase
{
    ...
    public ReportPdfByCoordinatorUseCase(
        ILoggedUser loggedUser,
        IConsultationsReadOnlyRepository consultationsReadOnlyRepository,
        IReadOnlyUsersRepository readOnlyUsersRepository
        )
    {
        _loggedUser = loggedUser;
        _readOnlyConsultationsRepository = consultationsReadOnlyRepository;
    ...
    public async Task<byte[]> Execute(long id)
    {
        ...
        foreach (var consultation in consultations)
        {
            ...
            {
                row.Cells[2].AddParagraph(consultation.Recommendations);
                SetStyleBaseForServiceInformation(row.Cells[2]);
            }

            if (string.IsNullOrWhiteSpace(consultation.Observation) == false)
            {
                row.Cells[3].AddParagraph(consultation.Observation);
                SetStyleBaseForServiceInformation(row.Cells[3]);
            }

            row.Cells[4].AddParagraph(consultation.Date.ToString("d"));
    ...
    private Document CreateDocument(string coordinatorName, string loggedUserName)
    {
        ...
        document.Info.Title = string.Format(ResourceReportsMessages.ALL_CONSULTATIONS_OFF_COORDINATOR, coordinatorName);
    ...
    private void CreateHeaderWithLogoAndText(Section page, string coordinatorName)
    {
        ...
        var title = string.Format(ResourceReportsMessages.ALL_CONSULTATIONS_OFF_COORDINATOR, coordinatorName);
    ...
    private Table CreateTable(Section page)
    {
        ...
        row.Cells[2].AddParagraph(ResourceReportsMessages.RECOMMENDATIONS);
